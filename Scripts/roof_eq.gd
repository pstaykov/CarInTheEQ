...
@export var width: float = 2.0        # total width across the car roof
@export var height_scale: float = 1.4
@export var floor_height: float = 0.1 # minimum bar height
...
func _ready() -> void:
	...
	for i in range(num_bars):
		...
		mesh.size = Vector3(bar_spacing * 0.8, floor_height, 1.5) # box: thinner z
		bar.mesh = mesh
